...
if (isset($location_mode))
{
    if ($location_mode == 'address')
    {
        ...
        } else
        {
            ...
            {
                $geocode = PluginusNet_GMapTargeting::file_get_contents_curl('https://maps.google.com/maps/api/geocode/json?address=' . $address . '&sensor=false');
            } else
            {
                //***
            }
...
<?php if ($mode == 'map'): ?>
    ...
    <div class="google_map" id="google_map_<?php echo $inique_id ?>" style="height: <?php echo $height ?>px; width: <?php echo $width_string ?>;"></div>
...
<?php else: ?>
    ...
    <img src="https://maps.googleapis.com/maps/api/staticmap?<?php echo $location_mode_string ?>&zoom=<?php echo $zoom ?>&maptype=<?php echo strtolower($maptype) ?>&size=<?php echo str_replace('%', '', $width) ?>x<?php echo str_replace('%', '', $height) ?><?php echo $marker_string ?>&sensor=false">
