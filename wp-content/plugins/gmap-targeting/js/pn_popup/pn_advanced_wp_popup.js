var PN_ADVANCED_WP_POPUP3 = function() {
    var self = {
        zindex: 1001,
        popup_li: null,
        params: null,
        init: function() {
            jQuery('body').prepend('<div id="pn_advanced_wp_popup3"></div>');
            jQuery('body').prepend('<div id="alert_advanced_wp_popup"></div>');
            jQuery('body').prepend('<div id="advanced_wp_popup_overlay"></div>');
            //***
            jQuery('.pn_ig_wp_popup_close3').life('click', function() {
        ...
        popup: function(params) {
            ...
            jQuery('#pn_advanced_wp_popup3').append('<div class="pn_advanced_wp_popup_li" style="width:' + params.width + 'px;margin-left:-' + params.width / 2 + 'px;">');
            this.popup_li = jQuery('#pn_advanced_wp_popup3 div:last-child');
            ...
            /***/
            jQuery(this.popup_li).find('.advanced_wp_popup_adv').html('<a title="www.pluginus.net on CodeCanyon" href="https://codecanyon.net/user/realmag777/portfolio?ref=realmag777" target="_blank"><img src="'+pn_gmt_plugin_url+'images/buy_on_codecanyon.jpg"></a>');
            if (params.title.length > 0) {
                popup.append('<div class="pn_titlebar" />');
                popup.children('.pn_titlebar').append('<h6>').children('h6').html(params.title);
                popup.children('.pn_titlebar').append('<a href="javascript:void(0);" class="pn_ig_wp_popup_close3"></a>');
            }
            /***/
        ...
        open: function(params, popup_li) {
            ...
            jQuery.each(params.buttons, function(index, button) {
                if (button.action == 'close') {
                    jQuery(popup_li).find('.advanced_wp_popup_buttons').append('<a href="javascript:void(0);" class="pn_button pn_ig_wp_popup_close3">' + button.name + '</a>');
                    return;
                }
                ...
                if (button.close) {
                    jQuery(popup_li).find('.advanced_wp_popup_buttons').append('<a href="javascript:pn_advanced_wp_popup3.do_action(' + index + ');void(0);" data-name="' + button.name + '" class="pn_button pn_ig_wp_popup_close3" style="display:' + button.display + ';">' + button.name + '</a>');
                } else {
                    jQuery(popup_li).find('.advanced_wp_popup_buttons').append('<a href="javascript:pn_advanced_wp_popup3.do_action(' + index + ');void(0);" data-name="' + button.name + '" class="button" style="display:' + button.display + ';">' + button.name + '</a>');
                }
        ...
        show_button: function(name) {
            jQuery('#pn_advanced_wp_popup3').find('.advanced_wp_popup_buttons').find("a[data-name*='" + name + "']").css('display', 'inline-block');
        },
...
var pn_advanced_wp_popup3 = null;
jQuery(document).ready(function() {
    pn_advanced_wp_popup3 = new PN_ADVANCED_WP_POPUP3();
    pn_advanced_wp_popup3.init();
});
